...
3. 账户的第二类：合约账户（contract account）
...
5. gas limit：Gas Limit 是用户为单笔交易或智能合约执行设置的最大 Gas 用量。它代表交易执行期间允许消耗的 Gas 上限，用于防止因代码错误或恶意合约导致无限循环等意外情况，避免耗尽资金。
